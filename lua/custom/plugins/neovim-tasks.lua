...
return {
  ...
  dependencies = {
    ...
    'mfussenegger/nvim-dap', -- Unlocks full debugging support (:Task debug)
    'nvim-telescope/telescope.nvim', -- Unlocks searchable task menus
  },
  cmd = {
    'Task',
    'TaskSetModuleParam',
  ...
  ft = { 'c', 'cpp', 'rust', 'go' }, -- Adjust based on your languages
  config = function()
    require('tasks').setup {
      ...
      -- dap_open_command = function() require('dap').continue() end,
    }
    ...
    -- Define global Keymaps to access every feature seamlessly
    ...
    vim.keymap.set('n', '<leader>ds', [[:Task start cmake debug<cr>]], { silent = true })
    ...
    vim.keymap.set('n', '<leader>cC', openCCMake, { silent = true, desc = 'Open ccmake in the terminal' })
    ...
    local function selectPreset()
      ...
      vim.ui.select(availablePresets, { prompt = 'Select build preset' }, function(choice, idx)
        ...
        if not projectConfig['cmake'] then projectConfig['cmake'] = {} end
    ...
    local function selectBuildKitOrPreset()
      if cmake_utils.shouldUsePresets() then
        ...
        require('tasks').set_module_param('cmake', 'build_kit')
      end
    end

    vim.keymap.set('n', '<leader>ck', selectBuildKitOrPreset, { silent = true, desc = 'Select The Build Kit' })

    local function selectBuildTypeOrPreset()
      if cmake_utils.shouldUsePresets() then
        selectPreset()
      else
        require('tasks').set_module_param('cmake', 'build_type')
      end
    end

    vim.keymap.set('n', '<leader>cv', selectBuildTypeOrPreset, { desc = 'Set The Build Type', silent = true })
    ...
    lualine.setup {
      sections = {
        ...
        lualine_x = { 'encoding', 'lsp_status', 'filetype', cmakeStatus },
      },
    }
  end,
}
